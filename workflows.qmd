...
Often we are (or think we are) only doing our data work on our own </br></br> When we want to facilitate collaboration or focus on reproducibility, we need new strategies </br></br> [What they Forgot to Teach You About R](https://rstats.wtf/) - Much of this thinking is distilled from this book.
...
Everything that really matters should be achieved through code that you save - Including creating figures - Contrast: Series of unrecorded mouse clicks

The **process** is important, the **product** is just an outcome - Outputs should be treated as disposable - Scripts are the source of truth
...
| Option | Persists? |
|-----------------------------------------------------|-------------------|
| A. `library(dplyr)` | [{{< fa check >}}]{style="color:green;"} |
| B. `summary <- head` | [{{< fa xmark >}}]{style="color:red;"} |
| C. `options(stringsAsFactors = FALSE)` | [{{< fa check >}}]{style="color:green;"} |
| D. `Sys.setenv(LANGUAGE = "fr")` | [{{< fa check >}}]{style="color:green;"} |
| E. `x <- 1:5` | [{{< fa xmark >}}]{style="color:red;"} |
| F. `attach(iris)` | [{{< fa check >}}]{style="color:green;"} |
...
By default, in the default library (base/recommended packages).
